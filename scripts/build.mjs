...
const infoPages = [
  ...
  },
  {
    ...
    content: `
      <h2>アクセス解析について</h2>
      <p>本サイトでは、閲覧状況とページ表示時のパフォーマンスを把握し、サイト改善に役立てるため、Cloudflare Web Analyticsを利用します。</p>
      <p>Cloudflare Web Analyticsは、ページ表示数や訪問状況などを計測するアクセス解析サービスです。Cloudflareの公式案内では、訪問者の個人データを収集または利用しない、プライバシーを重視したアクセス解析として説明されています。</p>
      <h2>広告と問い合わせについて</h2>
      <p>現在、本サイトでは広告配信サービスと問い合わせフォームを設置していません。</p>
